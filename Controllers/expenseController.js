...
exports.updateExpense = async (req, res) => {
  ...
  try {
    ...
    const { userIds = [], amount: updatedAmount, date, expense, currencyId } = fieldsToUpdate;
    ...
    const userIdsToAdd = userIds.filter((userId) => !existingUserIds.includes(userId));
    const userIdsToRemove = existingUserIds.filter((userId) => !userIds.includes(userId));
    ...
    if (userIdsToAdd.length > 0) {
      const splitAmount = updatedAmountValue / (userIds.length + 1);
    ...
    // Update all active split expenses
    const activeSplitExpenses = await findAllSplitExpense({ expId: id, isActive: true });
    const splitAmount = updatedAmountValue / activeSplitExpenses.length;
