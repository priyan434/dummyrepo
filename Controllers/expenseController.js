...
//     console.log("newRemoveUserIds:::",newRemoveUserIds);
...
exports.updateExpense = async (req, res) => {
  ...
  const updateExpenseSchema = Joi.object({
    ...
    currencyId: Joi.number().min(1).max(3).optional(),
    amount: Joi.number().min(1).optional(),
    userIds: Joi.array().items().optional(), // Only use userIds
  });

  if (process.env.DATABASE === "mongodb" && (!userId || !mongoose.Types.ObjectId.isValid(id))) {
    return res.status(404).json({
      message: INVALID_EXPENSE_ID.message,
      success: false,
      code: INVALID_EXPENSE_ID.code,
  ...
  if (!userId || !id || Object.keys(fieldsToUpdate).length === 0) {
    return res.status(400).send({
  ...
  try {
    ...
    if (error) {
      ...
      const errorMap = {
        ...
        expense: { "string.min": INVALID_EXPENSE_DETAILS, "string.pattern.base": EXPENSE_PATTERN_BASE, "string.empty": EMPTY_EXPENSE },
        currencyId: { "number.base": INVALID_BASECURRENCY_BASE, "number.min": INVALID_BASECURRENCY, "number.max": INVALID_BASECURRENCY },
      };
    ...
    if (!expenses || expenses.paidBy != userId) {
      return res.status(400).send({
        message: EXPENSE_NOT_FOUND.message,
        success: false,
        code: EXPENSE_NOT_FOUND.code,
    ...
    const { amount: updatedAmount, date, expense, currencyId, userIds = [] } = fieldsToUpdate;
    const updatedAmountValue = updatedAmount || expenses.amount;
    ...
    if (userIdsToRemove.length > 0) {
      ...
      await Promise.all(
        allExpense
          .filter((exp) => userIdsToRemove.includes(exp.userId))
          .map((exp) => updateExpense(exp, { isActive: false }))
      );
    }
    ...
    if (userIds.length > 0) {
      ...
      const splitAmount = updatedAmountValue / newNumberOfSplits;
      ...
      await Promise.all(
        allExpense.map((exp) =>
          updateExpense(exp, { myShare: splitAmount })
        )
      );
    }
    ...
    return res.status(200).json({
      ...
      code: UPDATE_EXPENSE.code,
      data: {
        expenses: await findExpenseById({ userId, expId: id, isActive: true }),
      },
    });
  } catch (error) {
    console.log(error);
    if (error.name === "CastError" || error.name === "SequelizeForeignKeyConstraintError") {
      return res.status(400).send({
        message: error.message,
        success: false,
        code: error.code || INVALID_INPUT.code,
      });
    }
    return res.status(500).send({
      message: SERVER_ERROR.message,
...
exports.updateExpense = async (req, res) => {
  ...
  try {
    ...
    if (fieldsToUpdate.userIds) {
      if (fieldsToUpdate.userIds.length > 0) {
        const usersExist = await checkUsersExist(fieldsToUpdate.userIds);
        if (!usersExist) {
          return res.status(400).send({
            message: "Invalid user IDs",
            success: false,
    ...
    const existingUserIds = activeSplitExpenses2.map((split) => split.userId);
    console.log("existingUserIds::::", existingUserIds);
    
    // Filter out those IDs which are in existingUserIds but not in userIds
    const newRemoveUserIds = existingUserIds.filter(existingId => !userIds.includes(existingId) && existingId !== userId);
    console.log("newRemoveUserIds:::", newRemoveUserIds);
    ...
    if (userIds.length > 0) {
      ...
      const existingUserIds = activeSplitExpenses.map((split) => split.userId);
